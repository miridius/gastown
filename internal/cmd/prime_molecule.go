...
func buildRefineryPatrolVars(ctx RoleContext) []string {
	...
	if rigCfg != nil && rigCfg.Beads != nil && rigCfg.Beads.Prefix != "" {
		...
		if issue, err := bd.Show(rigBeadID); err == nil {
			...
			for _, key := range []string{"integration_branch_refinery_enabled", "integration_branch_auto_land", "run_tests", "delete_merged_branches", "judgment_enabled", "review_depth", "setup_command", "typecheck_command", "lint_command", "test_command", "build_command", "merge_strategy", "require_review"} {
