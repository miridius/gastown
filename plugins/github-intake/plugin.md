+++
name = "github-intake"
description = "Poll GitHub issues, sanitize content, triage, and convert to Gas Town work items"
version = 1
...
[tracking]
labels = ["plugin:github-intake", "category:issue-intake"]
digest = true
...
Polls GitHub for new issues, sanitizes content against prompt injection,
triages by author, and converts approved issues to Gas Town work items.
...
Query for open issues that haven't been processed yet. We track processed
issues by looking for beads with the `github-intake` label that reference
...
Check which issues have already been converted to beads:
...
For each unprocessed issue, sanitize content and triage based on author.

**CRITICAL: Sanitization happens HERE, before any content enters agent context.**

The sanitization rules (applied to both title and body):
...
```bash
# Find beads that were flagged for review and have been updated
...
On failure:
```bash
bd create "github-intake: FAILED" -t chore --ephemeral \
  -l type:plugin-run,plugin:github-intake,result:failure \
  -d "GitHub intake failed: $ERROR" --silent 2>/dev/null || true

gt escalate "Plugin FAILED: github-intake" \
  --severity medium \
  --reason "$ERROR"
```
