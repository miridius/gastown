...
func TestDefaultConfig_MaxConnections(t *testing.T) {
	...
	if config.MaxConnections != 150 {
		t.Errorf("DefaultMaxConnections = %d, want 150", config.MaxConnections)
	}
}
...
func TestFindBrokenWorkspaces_HealthyWorkspace(t *testing.T) {
	...
	// Database exists with valid noms/manifest
	nomsDir := filepath.Join(townRoot, ".dolt-data", "hq", ".dolt", "noms")
	if err := os.MkdirAll(nomsDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(nomsDir, "manifest"), []byte("5:5"), 0644); err != nil {
		t.Fatal(err)
	}
...
func TestFindBrokenWorkspaces_MultipleRigs(t *testing.T) {
	...
	rigBNomsDir := filepath.Join(townRoot, ".dolt-data", "rig-b", ".dolt", "noms")
	if err := os.MkdirAll(rigBNomsDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(rigBNomsDir, "manifest"), []byte("5:5"), 0644); err != nil {
		t.Fatal(err)
	}
